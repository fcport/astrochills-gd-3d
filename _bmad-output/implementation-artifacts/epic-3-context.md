...
- **Dipendenze in avanti note dall'analisi di readiness (da chiudere a monte dell'implementazione):**
  ...
  - La firma `wait_activity` va portata alla coppia `started`/`ended` prima di implementare le attività (C4), e serve un proprietario esplicito per la telemetria (C3), plausibilmente un autoload `autoloads/telemetry.gd` che non passa da `Log`.
