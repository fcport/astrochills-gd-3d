...
## chiamate `Events.wait_activity_*` e gestisce il timer; la funzione pura non lo tocca.
##
...
enum Step {
	...
	BREWING,  ## sul fuoco: si aspetta (inerte all'interazione)
...
## Tarabile guardando: è una verifica d'operatore, non un numero da stimare.
...
func _ready() -> void:
	...
	# conta col tempo scalato, così F1–F4 lo accelerano. La durata la fissa il codice (non la scena) perché è un
...
## oltre a `enabled`, tace durante l'attesa (BREWING) — si aspetta, non si clicca. Da non-posseduta `enabled` è già falso (la moka nasce spenta e invisibile).
...
# consulta e ci appende gli effetti (timer, telemetria); la funzione non tocca
# nulla di tutto ciò.
...
static func next_on_interact(step: Step) -> Step:
	match step:
		...
		Step.FILLED:
			return Step.BREWING  # il nodo avvia il timer
		Step.READY:
			return Step.POURED
		Step.POURED:
...
func _on_interacted(_by: Node3D) -> void:
	...
	# FILLED → BREWING: parte l'attesa reale. Il resto delle
	# transizioni è solo un cambio di prompt, che `prompt()` riflette da sé.
	if was_filled and _step == Step.BREWING:
		_start_brewing()


## Sul fuoco: avvia il timer d'attesa. `can_interact()` è già falso (BREWING), quindi il
## prompt sparisce da sé al prossimo tick del giocatore.
func _start_brewing() -> void:
	_brew_timer.start()


## L'attesa è conclusa: il caffè è pronto. BREWING → READY è guidato da QUI, non da
## un'interazione.
